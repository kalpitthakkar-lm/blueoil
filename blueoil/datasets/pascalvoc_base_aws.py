...
class PascalvocBase(ObjectDetectionBase):
    ...
    def _get_boxes_from_annotation(self, xml_file):
        ...
        bboxes = []
        # Map both masked classes to same ID
        classes_idx_map = {
            'face': 0,
            'masked_face': 1,
            'colormasked_face': 2,
        ...
        for obj in root.iter('object'):
            ...
            class_index = classes_idx_map[class_name]

            for e in list(obj):
                if e.tag == "bndbox":
                    # subtract 1 to make boxes 0-based coordinate.
                    # Kalpit: First to float to be more general.
                    ymin = int(float(e.find("ymin").text)) - 1
                    xmin = int(float(e.find("xmin").text)) - 1
                    ymax = int(float(e.find("ymax").text)) - 1
                    xmax = int(float(e.find("xmax").text)) - 1

                    w = xmax - xmin
                    h = ymax - ymin
                    bbox = [xmin, ymin, w, h, class_index]
                    bboxes.append(bbox)
                    # Add the face object twice every time you see it
                    if class_index == 0:
                        bboxes.append(bbox)
